...
const display_next_question = () => 
new Promise(resolve => {
  // document.body.addEventListener("keydown", e => {
    // if (index )
    if (document.querySelector(".next_button").style.opacity == "1") 
    resolve();
    // return;
  // });
...
function ask_question(){
  ...
  answers.forEach(e => {
    ...
    btn.innerHTML = e.trim();
    btn.id = (btn.innerHTML == correct_ans);
    btn.addEventListener("click", () => {
      if (btn.innerHTML == correct_ans){
        score++;
        document.querySelector(".score_display").innerHTML = `Score: ${score}/10`;
        btn.style.background = "rgb(18, 180, 18)";
      } else {
        if (document.body.contains(document.querySelector("#true")))
...
function set_loading(button_class){
  if (document.querySelector(button_class).innerHTML == "Getting the quiz.") {
    document.querySelector(button_class).innerHTML = "Getting the quiz..";
  } else if (document.querySelector(button_class).innerHTML == "Getting the quiz..") {
    document.querySelector(button_class).innerHTML = "Getting the quiz...";
  } else if (document.querySelector(button_class).innerHTML == "Getting the quiz...") {
    document.querySelector(button_class).innerHTML = "Getting the quiz.";
  } else {
